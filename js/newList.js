...
$(function() {
    ...
    var keyWordId = sessionStorage.getItem('keyWordId');
    $.ajax({
      url:api+'indexIsCart',
    ...
    function head(mon,day,week,year,value){
      ...
      $('.list_data').each(function(index,val){
        ...
        $(this).text(mon >= 10? mon +"-"+ (day>=10?day : '0'+String(day)) : '0'+String(mon) +"-"+ (day>=10?day : '0'+String(day)))
        if(index==1){
    ...
    function reloads(result){
      ...
      // console.log(date)
      $.ajax({
        url:api+'indexAppointSelectStore',
        headers:{'userauthkey':token},
        beforeSend:function(){
            $('.loading').show();
            $('.mui-scroll-wrapper').hide();
    ...
    $(".timer_content").on("tap",'.reserve_color', function() {
      ...
      var time_id = $(this).parents('.contentList').data('timeinfo'); // 获得时间id
      ...
      dtpicker.show(function(e) {
        ...
        var listObj = {listOrdertime:listOrdertime,clickTime:clickTime,store_id:store_id,cartName:cartName,cartSite:cartSite,time_id:time_id}
